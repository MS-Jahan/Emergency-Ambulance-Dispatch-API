...
var require_client = __commonJS({
  "node_modules/.prisma/client/index.js"(exports) {
    ...
    exports.Prisma.RequestStatusLogScalarFieldEnum = {
      id: "id",
      requestId: "requestId",
      actorId: "actorId",
...
var createDriverSchema = z5.object({
  name: z5.string().min(2).max(80),
  email: z5.string().email().max(200),
  password: z5.string().min(8, "Password must be at least 8 characters").max(72).regex(/[A-Za-z]/, "Password must contain a letter").regex(/[0-9]/, "Password must contain a number"),
  phone: z5.string().min(6).max(20),
  licenseNumber: z5.string().min(4).max(40)
});
var listUsersQuerySchema = paginationQuerySchema.extend({
  role: z5.enum(["PATIENT", "DRIVER", "ADMIN"]).optional(),
  q: z5.string().max(120).optional()
});
var updateUserRoleSchema = z5.object({
  role: z5.enum(["PATIENT", "DRIVER", "ADMIN"])
});
var listAuditLogsQuerySchema = paginationQuerySchema.extend({
  requestId: z5.string().cuid().optional()
});
...
var createAmbulanceSchema = z6.object({
  plateNumber: z6.string().min(4).max(20).regex(/^[A-Za-z0-9-]+$/, "Plate number can only contain letters, numbers, and dashes"),
  type: z6.enum(["BASIC", "ICU", "CARDIAC"]).default("BASIC"),
  homeHospitalId: z6.string().cuid().optional(),
  status: z6.enum(["AVAILABLE", "ON_TRIP", "MAINTENANCE"]).optional()
});
var updateAmbulanceSchema = z6.object({
  type: z6.enum(["BASIC", "ICU", "CARDIAC"]).optional(),
  homeHospitalId: z6.string().cuid().nullable().optional(),
  status: z6.enum(["AVAILABLE", "ON_TRIP", "MAINTENANCE"]).optional()
});
var listAmbulancesQuerySchema = paginationQuerySchema.extend({
  q: z6.string().max(20).optional(),
  status: z6.enum(["AVAILABLE", "ON_TRIP", "MAINTENANCE"]).optional(),
  type: z6.enum(["BASIC", "ICU", "CARDIAC"]).optional()
});
var nearbyQuerySchema = z6.object({
  lat: z6.coerce.number().min(-90).max(90),
  lng: z6.coerce.number().min(-180).max(180),
  radiusKm: z6.coerce.number().min(0.5).max(50).default(10),
  type: z6.enum(["BASIC", "ICU", "CARDIAC"]).optional()
});

// src/modules/ambulances/ambulance.routes.ts
var ambulancesRouter = Router2();
...
async function loginUser(input) {
  ...
  const passwordMatches = await bcrypt2.compare(input.password, user.password);
  if (!passwordMatches) {
    throw AppError.unauthorized("Invalid email or password");
  }
...
var registerSchema = z7.object({
  name: z7.string().min(2).max(80),
  email: z7.string().email().max(200),
  password: z7.string().min(8, "Password must be at least 8 characters").max(72).regex(/[A-Za-z]/, "Password must contain a letter").regex(/[0-9]/, "Password must contain a number"),
  phone: z7.string().min(6).max(20).optional()
});
var loginSchema = z7.object({
  email: z7.string().email(),
  password: z7.string().min(1)
});
var googleLoginSchema = z7.object({
  idToken: z7.string().min(20)
});
var refreshTokenSchema = z7.object({
  refreshToken: z7.string().min(20)
});
...
var updateDriverStatusSchema = z8.object({
  status: z8.enum(["AVAILABLE", "ON_TRIP", "OFFLINE"])
});
var updateDriverLocationSchema = z8.object({
  lat: z8.number().min(-90).max(90),
  lng: z8.number().min(-180).max(180)
});

// src/modules/drivers/driver.routes.ts
var driverRouter = Router4();
...
var createFeedbackSchema = z9.object({
  requestId: z9.string().cuid(),
  rating: z9.number().int().min(1).max(5),
  comment: z9.string().max(500).optional()
});

// src/modules/feedback/feedback.routes.ts
var feedbackRouter = Router5();
...
var createHospitalSchema = z10.object({
  name: z10.string().min(2).max(120),
  address: z10.string().min(5).max(250),
  lat: z10.number().min(-90).max(90),
  lng: z10.number().min(-180).max(180),
  phone: z10.string().min(6).max(20)
});
var updateHospitalSchema = createHospitalSchema.partial();
var listHospitalsQuerySchema = paginationQuerySchema.extend({
  q: z10.string().max(120).optional()
});

// src/modules/hospitals/hospital.routes.ts
var hospitalsRouter = Router6();
...
var initiatePaymentSchema = z11.object({
  requestId: z11.string().cuid()
});
var listPaymentsQuerySchema = paginationQuerySchema.extend({
  status: z11.enum(["PENDING", "PAID", "FAILED", "REFUNDED"]).optional()
});
...
var createRequestSchema = z12.object({
  pickupAddress: z12.string().min(5).max(250),
  pickupLat: z12.number().min(-90).max(90),
  pickupLng: z12.number().min(-180).max(180),
  priority: z12.enum(["CRITICAL", "HIGH", "NORMAL"]).default("NORMAL"),
  destinationHospitalId: z12.string().cuid().optional()
});
var listRequestsQuerySchema = paginationQuerySchema.extend({
  ...
  priority: z12.enum(["CRITICAL", "HIGH", "NORMAL"]).optional(),
  q: z12.string().max(250).optional()
});
var assignRequestSchema = z12.object({
  ambulanceId: z12.string().cuid(),
  driverId: z12.string().cuid().optional()
});
var updateStatusSchema = z12.object({
  status: z12.enum(["EN_ROUTE_PICKUP", "PICKED_UP", "EN_ROUTE_HOSPITAL", "COMPLETED"]),
  note: z12.string().max(250).optional(),
  destinationHospitalId: z12.string().cuid().optional()
});
var cancelRequestSchema = z12.object({
  cancelReason: z12.string().min(3).max(250)
});

// src/modules/requests/request.routes.ts
var requestsRouter = Router8();
